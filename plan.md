...
| PR-2 | `feat/worker-core` | `worker/` | Worker 全部后端:路由、Graph 客户端、双认证策略、token 缓存轮换、账号注册表、密码门端点;vitest 单测(mock Graph) | docs/api.md | 待认领 |
| PR-3 | `feat/account-cli` | `scripts/` | 账号管理 CLI:add(个人版设备码流/企业版密钥验证并解析 driveId)、list、remove,经 wrangler 写 KV | docs/api.md §KV | 待认领 |
